...
export = {
    view: (vnode: m.Vnode<IBadRowsSummary>) => m('div.modal',
        { className: vnode.attrs.modal === 'stream' ? 'is-active' : 'is-inactive' },
        [
            ...
            m('form.modal-card', {
                onsubmit: (e: Event) => {
                    ...
                    const esUrl = new URL((fields.namedItem('stream-endpoint') as HTMLInputElement).value);

                    const pathparts = esUrl.pathname.split('/').filter((x) => !!x);
                    pathparts.push((fields.namedItem('stream-index') as HTMLInputElement).value);
                    pathparts.push('_search');

                    esUrl.pathname = pathparts.join('/');
                    esUrl.searchParams.set('size', '50');
                    esUrl.searchParams.set('q', (fields.namedItem('stream-query') as HTMLInputElement).value || '*');
                    ...
                    let timeoutId = window.setTimeout(function pollStream() {
                        m.request({
                            ...
                            if (hits.length) {
                                vnode.attrs.addRequests('Live Data',
                ...
                [
                    ...
                    m('section.modal-card-body', [
                        ...
                        m('.field', [
                            ...
                            m('.control', m('input#stream-endpoint.input[name=endpoint][type=url][required]', {
                                placeholder: 'http://es.example.com',
                            })),
                        ]),
                        m('.field', [
                            m('label.label[for=stream-index]', 'ElasticSearch Index'),
                            m('.control', m('input#stream-index.input[name=index][type=text][required]', {
                                placeholder: 'good',
                            })),
                        ]),
                        m('.field', [
                            m('label.label[for=stream-query]', 'Filter Query'),
                            m('.control', m('input#stream-query.input[name=query][type=text]', {
                                placeholder: '*',
                            })),
                        ]),
                    ]),

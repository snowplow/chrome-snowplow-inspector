import { default as m, redraw, Component } from "mithril";
...
export const EditTestSuites: Component<EditTestSuitesOptions> = {
  view: ({ attrs: { setModal, setSuites, suites } }) =>
    m("div.modal.is-active", [
      m("div.modal-background", { onclick: () => setModal() }),
      m("div.modal-card", [
        m("header.modal-card-head", [
          m("p.modal-card-title", "Edit Test Suites"),
        ...
        m(
          ...
          m(
            "form#edit-testsuites.form.testsuite-definition",
            {
        ...
        m(
          ...
          m(
            "button.button[form=edit-testsuites][name=save-testsuites]",
            "Save Test Suites"
          ),
          m(
            ...
            {
              onclick: () => {
                suites.push(JSON.parse(JSON.stringify(SUITE_TEMPLATE)));
              },
            },
            "New Suite"
          ),
